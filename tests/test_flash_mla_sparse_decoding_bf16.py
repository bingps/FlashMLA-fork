                ...
                for have_topk_len in [False, True]:
                    for h_q in [64, 128]:
                        cur_correctness_cases = [
                            RawTestParam(
                                b, h_q, s_q, 1, s_k, is_varlen, topk,
...
def run_one_case(p: TestParam) -> Result:
    ...
    def run_decode():
        return lib.run_flash_mla_decode(p, t, tile_scheduler_metadata, None, is_fp8_kvcache=False)
    ...
    if p.num_runs > 0:
        ...
        def pick_kernel_time_usage(kernel_name: str):
            matches = [kernel_name in s for s in result.get_kernel_names()]
            if any(matches):
        ...
        flops_and_mem_vol = lib.count_flop_and_mem_vol_for_decode(p, t, is_fp8_kvcache=False)
        ...
        performance_result = Result(
            True,
            theoritical_compute_memory_ratio,
            e2e_time_usage_us,
            kernel_time_usages_us[splitkv_kernel_name] or 0.0,
    ...
    if p.check_correctness:
        torch.cuda.synchronize()
        with torch.profiler.record_function("reference_flash_mla_bf16"):
            out_ref, lse_ref = ref.ref_sparse_attn_decode(p, t)

        is_out_correct = kk.check_is_allclose("out", out_ans, out_ref, abs_tol=2e-3, rel_tol=2.5 / 128, cos_diff_tol=1e-5)
        is_lse_correct = kk.check_is_allclose("lse", lse_ans, lse_ref, abs_tol=1e-5, rel_tol=1e-4)
...
def test_flash_mla_sparse_decoding_bf16():
